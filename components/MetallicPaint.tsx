...
export async function parseLogoImage(file: File): Promise<ParsedLogo> {
    ...
    return new Promise((resolve, reject) => {
        ...
        img.onload = function onLoad() {
            ...
            if (!shapeCtx) {
                reject(new Error('Failed to get 2D context for shape canvas'));
                return;
            }
            ...
            for (let y = 0; y < height; y++) {
                for (let x = 0; x < width; x++) {
                    ...
                    if (!shapeMask[idx]) {
                        ...
                        const raw = maxVal ? u[idx] / maxVal : 0;
...
export default function MetallicPaint({ imageData, params = defaultParams, className }: MetallicPaintProps) {
    ...
    }, [gl, uniforms, params.edge, params.liquid, params.patternBlur, params.patternScale, params.refraction]);
    ...
    useEffect(() => {
        function initShader() {
            ...
            function getUniforms(programRef: WebGLProgram, ctx: WebGL2RenderingContext) {
                const uniformRecord: Record<string, WebGLUniformLocation | null> = {};
                const uniformCount = ctx.getProgramParameter(programRef, ctx.ACTIVE_UNIFORMS) as number;
                for (let i = 0; i < uniformCount; i++) {
                    const uniformName = ctx.getActiveUniform(programRef, i)?.name;
                    if (!uniformName) continue;
                    uniformRecord[uniformName] = ctx.getUniformLocation(programRef, uniformName);
                }
                return uniformRecord;
            }
            const foundUniforms = getUniforms(program, context);
            setUniforms(foundUniforms);
        ...
        initShader();
        // uniforms will be updated by the dedicated effect once the WebGL context is ready
    }, []);
    ...
    useEffect(() => {
        ...
        function render(currentTime: number) {
            ...
            if (uniforms.u_time) glContext.uniform1f(uniforms.u_time, totalAnimationTime.current);
    ...
    const composedClassName = ['paint-container', className].filter(Boolean).join(' ');
